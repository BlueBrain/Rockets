...
#ifndef ROCKETS_JSONRPC_RECEIVER_INTERFACE_H
#define ROCKETS_JSONRPC_RECEIVER_INTERFACE_H
...
{
/**
 * A base class for receiver implementations which provides synchronous and
 * asynchronous processing of JSON-RPC requests.
 *
 * A concrete implementation must provide the actual processing of JSON-RPC
 * request, mainly to not leak a JSON library implementation.
 */
...
private:
     ...
     * @param callback that return a json response string in JSON-RPC 2.0
     *        format upon request completion.
     * @sa RequestProcessor::process
     */
    virtual void _process(const Request& request,
                          AsyncStringResponse callback) = 0;

    /**
     * Callback for each method added by bind() or connect() to remember it for
     * processing or to reject the method by throwing a std::invalid_argument
     * exception.
     */
    virtual void _registerMethod(const std::string& method,
                                 DelayedResponseCallback action) = 0;
};
}
}
